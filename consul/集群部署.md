...
```sh
# consul-client-1
docker run -d -p 8710:8500 --restart=always -v /root/data/consul/client1/data:/consul/data -v /root/data/consul/client1/config:/consul/config -e CONSUL_BIND_INTERFACE='eth0' --privileged=true --name=consul_client_1 consul agent -bootstrap-expect=3 -ui -node=consul_client_1 -client='0.0.0.0' -data-dir /consul/data -config-dir /consul/config -datacenter=cdd-dc-dev -join=$JOIN_IP
```

```sh
# consul-client-2
docker run -d -p 8720:8500 --restart=always -v /root/data/consul/client2/data:/consul/data -v /root/data/consul/client2/config:/consul/config -e CONSUL_BIND_INTERFACE='eth0' --privileged=true --name=consul_client_2 consul agent -bootstrap-expect=3 -ui -node=consul_client_2 -client='0.0.0.0' -data-dir /consul/data -config-dir /consul/config -datacenter=cdd-dc-dev -join=$JOIN_IP
```
